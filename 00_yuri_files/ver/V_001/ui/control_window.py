...
class ControlWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle(
            "NIC Forensic Analysis Tool v1.0.5 - Painel de Controle")
        self.resize(700, 950)
    ...
    def export_full_report(self, content):
        ...
        file_name, _ = QFileDialog.getSaveFileName(
            self, "Salvar", "MediaInfo.txt", "Texto (*.txt)", options=options)
        if file_name:
    ...
    def export_tab_content(self, content, title):
        ...
        file_name, _ = QFileDialog.getSaveFileName(
            self, "Salvar", f"analise_{safe_title}.txt", "Texto (*.txt)")
        if file_name:
            with open(file_name, 'w', encoding='utf-8') as f:
                f.write(content)
