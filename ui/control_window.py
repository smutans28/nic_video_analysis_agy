...
from modules.version_checker import VersionCheckWorker
...
class ControlWindow(QMainWindow):
    def __init__(self):
        ...
        lbl_title = QLabel("NIC FORENSIC VIDEO TOOL v1.0.7")
        ...
        self.chk_info = QCheckBox(
            "Informações Gerais / Metadados (Container/Stream/CODEC)")
    ...
    def show_about_dialog(self):
        ...
        text = """
        <h3>NIC FORENSIC VIDEO TOOL v51.0.7</h3>
            ...
            try:
                ...
                if "Erro" not in res["simplified"]:
                    self.create_result_tab(
                        "Metadados - Gerais (MediaInfo)",
                        res["simplified"],
                        res.get("media_text", ""))
                    
                    self.create_result_tab(
                        "Metadados - Gerais (ExifTool)",
                        {"Relatório Estrutural": "--- AVISO FORENSE ---\nOs metadados abaixo foram extraídos pelo ExifTool, incluindo campos proprietários de fabricantes (Câmeras, GPS, Edição).\n\n" + res.get("exif_text_basic", "[NENHUM DADO ENCONTRADO]")},
                        res.get("exif_text_full", ""))
                else:
    ...
    def start_gop_analysis(self):
        ...
        worker.error.connect(self._on_gop_error)
    ...
    def start_deep_gop_analysis(self):
        ...
        worker.finished.connect(self._on_worker_finished)
        worker.error.connect(self._on_deep_gop_error)
        self.workers.append(worker)
        worker.start()
    ...
    @Slot(str)
    def _on_gop_error(self, e):
        QMessageBox.critical(self, "Erro GOP", e)

    @Slot(str)
    def _on_deep_gop_error(self, e):
        QMessageBox.critical(
            self,
            "Erro Deep GOP",
            f"Falha no FFprobe (verifique se está instalado):\n{e}")

    # --- GATILHOS DE EXPORTAÇÃO (Refatoração Definitiva de Memória para Nuitka) ---
    # Nuitka C++ costuma destruir closures (lambdas, partials) criadas dentro de loops de UI.
    # Ele também limpa dinâmicas como btn.property().
    # A única arquitetura resiliente é salvar o conteúdo pesado em um Dicionário nativo Python
    # e amarrar os botões via seu objectName() seguro do Qt, chamando slots sem parâmetros.
    
    @Slot()
    def _on_export_full_clicked(self):
        btn = self.sender()
        if not btn: return
        btn_id = btn.objectName()
        if btn_id in self.report_payloads:
            payload, r_type = self.report_payloads[btn_id]
            self.export_full_report(payload, r_type)

    @Slot()
    def _on_export_tab_clicked(self):
        btn = self.sender()
        if not btn: return
        btn_id = btn.objectName()
        if btn_id in self.tab_payloads:
            payload, title = self.tab_payloads[btn_id]
            self.export_tab_content(payload, title)
    ...
    def create_result_tab(self, title, data_dict, full_mediainfo_text=None):
        ...
        if full_mediainfo_text:
            ...
            # Ajusta o título do botão dinamicamente
            btn_label = "Exportar Relatório COMPLETO (TXT)"
            if "mediainfo" in title.lower():
                btn_label = "Exportar Relatório Completo MediaInfo (TXT)"
            elif "exiftool" in title.lower():
                btn_label = "Exportar Relatório Completo ExifTool (TXT)"
                
            btn_export_full = QPushButton(btn_label)
            btn_export_full.setStyleSheet(
                "background-color: #3f6f96; color: white;")
            btn_export_full.setProperty("full_mediainfo_text", full_mediainfo_text)
            
            # Define o tipo de relatório para o nome do arquivo dinâmico
            if "exiftool" in title.lower():
                report_type = "exiftool"
            else:
                report_type = "mediainfo"
                
            # Associa um ID único e seguro
            btn_id = f"btn_full_{title.replace(' ', '_')}"
            btn_export_full.setObjectName(btn_id)
            
            # Armazena na memória nativa do Python e dispara sinal cego (inquebrável no C++)
            if not hasattr(self, 'report_payloads'): self.report_payloads = {}
            self.report_payloads[btn_id] = (full_mediainfo_text, report_type)
            
            btn_export_full.clicked.connect(self._on_export_full_clicked)
            export_layout.addWidget(btn_export_full)
            export_layout.addStretch()
            layout.addLayout(export_layout)

        btn_export_tab = QPushButton(f"Exportar Conteúdo da Aba ({title})")
        btn_tab_id = f"btn_tab_{title.replace(' ', '_')}"
        btn_export_tab.setObjectName(btn_tab_id)
        
        if not hasattr(self, 'tab_payloads'): self.tab_payloads = {}
        self.tab_payloads[btn_tab_id] = (content, title)
        
        btn_export_tab.clicked.connect(self._on_export_tab_clicked)
        layout.addWidget(btn_export_tab)
        self.tabs_results.addTab(tab, title)

    def export_full_report(self, content, report_type="mediainfo"):
        # Gera prefixo baseado no arquivo atual
        if self.current_video_path:
            base_name = os.path.basename(self.current_video_path)
            name_no_ext, _ = os.path.splitext(base_name)
            default_name = f"{name_no_ext}_{report_type}_completo.txt"
        else:
            default_name = f"{report_type.capitalize()}_Completo.txt"
            
        file_name, _ = QFileDialog.getSaveFileName(
            self, f"Salvar Relatório {report_type.capitalize()}", default_name, "Texto (*.txt)")
            
        if file_name:
            with open(file_name, 'w', encoding='utf-8') as f:
                # str() cast pro caso de Nuitka empacotar QString diferente
                f.write(str(content))
    ...
    def check_for_updates(self):
        # A versão atual do programa lida pela janela
        current_version = "1.0.7"
        self.update_worker = VersionCheckWorker(current_version)
        self.update_worker.finished.connect(self.on_update_check_finished)
        self.update_worker.start()
        
    @Slot(bool, str, str)
    def on_update_check_finished(self, has_update, online_version, download_url):
        if has_update:
            # Mostra no topo da janela temporariamente pra avisar
            msg = QMessageBox(self)
            msg.setIcon(QMessageBox.Icon.Information)
            msg.setWindowTitle("Atualização Disponível")
            msg.setText(f"Uma versão mais nova ({online_version}) está disponível no Servidor!\n\nA versão instalada é a 1.0.7.")
            
            # Botão Opcional para abrir o link
            btn_download = msg.addButton("Baixar Agora", QMessageBox.ButtonRole.ActionRole)
            msg.addButton("Lembrar Depois", QMessageBox.ButtonRole.RejectRole)
            
            # Usa getattr para garantir isolamento limpo
            msg.exec()
